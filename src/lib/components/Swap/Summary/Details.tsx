  ...
  const details = useMemo(() => {
    ...
    if (feeOptions) {
      ...
      if (fee.greaterThan(0)) {
        ...
        rows.push([`${integrator} fee`, `${parsedFee} ${outputCurrency.symbol || currencyId(outputCurrency)}`])
      }
    }

    if (impact) {
      rows.push([`Price impact`, impact.toString(), impact.warning])
    }

    if (lpFeeAmount) {
      const parsedLpFee = formatCurrencyAmount(lpFeeAmount, 6, i18n.locale)
      rows.push([`Liquidity provider fee`, `${parsedLpFee} ${inputCurrency.symbol || currencyId(inputCurrency)}`])
    }

    if (trade.tradeType === TradeType.EXACT_OUTPUT) {
      const localizedMaxSent = formatCurrencyAmount(trade.maximumAmountIn(slippage.allowed), 6, i18n.locale)
      rows.push([`Maximum sent`, `${localizedMaxSent} ${inputCurrency.symbol}`])
    }

    if (trade.tradeType === TradeType.EXACT_INPUT) {
      const localizedMaxSent = formatCurrencyAmount(trade.minimumAmountOut(slippage.allowed), 6, i18n.locale)
      rows.push([`Minimum received`, `${localizedMaxSent} ${outputCurrency.symbol}`])
    }

    rows.push([`Slippage tolerance`, `${slippage.allowed.toFixed(2)}%`, slippage.warning])
