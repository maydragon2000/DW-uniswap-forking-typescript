  ...
  const approvalAction = useMemo((): Action | undefined => {
    ...
    switch (approvalState) {
      case ApproveOrPermitState.REQUIRES_APPROVAL:
        if (isPending) {
          return { message: "Approve in your wallet", icon: Spinner }
        }
        return {
          message: <>Approve {currency.symbol} first</>,
          onClick: onApprove,
          children: <>Approve</>,
        }
      case ApproveOrPermitState.REQUIRES_SIGNATURE:
        if (isPending) {
          return { message: "Allow in your wallet", icon: Spinner }
        }
        return {
          message: <>Allow {currency.symbol} first</>,
          onClick: onApprove,
          children: <>Allow</>,
        }
      ...
      case ApproveOrPermitState.PENDING_SIGNATURE:
        return { message: "Allowance pending", icon: Spinner }
